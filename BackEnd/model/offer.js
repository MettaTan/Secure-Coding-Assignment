...
var offerDB = {
    ...
    AcceptOrRejectOffer: function (status,offerid,userid, callback) {
        var conn = db.getConnection();                                                                                                             
        ...
        conn.connect(function (err) {
            if (err) {
                ...
                var sql = 'select l.fk_poster_id from listings l, offers o where l.id = o.fk_listing_id and o.id=?;';
                conn.query(sql, [offerid], function (err, result) {
                    console.log(result[0].fk_poster_id)
                    console.log(userid)
